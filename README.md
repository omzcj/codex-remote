...
`CODEX_APP_SERVER_USE_LOCAL_DAEMON=1` 的 macOS 环境。它不安装常驻 LaunchAgent，不会在
`start` 中隐式安装、bootstrap 或升级 Codex。
...
相同的 Desktop 版本例外。`start` 在自动清理运行状态后还会再次 preflight，因为 ChatGPT 可能
